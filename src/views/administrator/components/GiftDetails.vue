    ...
    <template slot-scope="scope">
      <el-form label-position="left" inline class="demo-table-expand">
        <el-form-item label="礼品 ID">
          <span>{{ fromData[scope.$index].GiftUnique }}</span>
        </el-form-item>
        <el-form-item label="轮播图片">
          <div
            v-for="(value, index) in fromData[scope.$index].CarouselPictures"
            :key="value.id"
        ...
        <el-form-item label="规格">
          <div
            v-for="(value, index) in fromData[scope.$index].Specification"
            :key="value.id"
        ...
        <el-form-item label="介绍图片">
          <div
            v-for="(value, index) in fromData[scope.$index].IntroduceImg"
            :key="value.id"
            class="IntroduceImgDiv"
          >
        ...
        <el-form-item label="礼品全名">
          <p style="display: inline-block" v-if="scope.row.Edit">
            <el-input
              v-model="fromData[scope.$index].CommodityFunllName"
              placeholder="请输入内容"
              @change="
                GiftBasicInformationChange(
                  'CommodityFunllName',
                  scope.$index,
                  fromData[scope.$index].CommodityFunllName
                )
              "
            ></el-input>
          </p>
          <p style="display: inline-block" v-else>
            <span>{{ fromData[scope.$index].CommodityFunllName }}</span>
          </p>
        </el-form-item>
        <el-form-item label="礼品缩略名">
          <p style="display: inline-block" v-if="scope.row.Edit">
            <el-input
              v-model="fromData[scope.$index].CommodityName"
              placeholder="请输入内容"
              @change="
                GiftBasicInformationChange(
                  'CommodityName',
                  scope.$index,
                  fromData[scope.$index].CommodityName
                )
              "
            ></el-input>
          </p>
          <p style="display: inline-block" v-else>
            <span>{{ fromData[scope.$index].CommodityName }}</span>
          </p>
        </el-form-item>
        <el-form-item label="企业名">
          <p style="display: inline-block" v-if="scope.row.Edit">
            <el-input
              v-model="fromData[scope.$index].BusinessName"
              placeholder="请输入内容"
              @change="
                GiftBasicInformationChange(
                  'BusinessName',
                  scope.$index,
                  fromData[scope.$index].BusinessName
                )
              "
            ></el-input>
          </p>
          <p style="display: inline-block" v-else>
            <span>{{ fromData[scope.$index].BusinessName }}</span>
          </p>
        </el-form-item>
        <el-form-item label="营业注册号">
          <p style="display: inline-block">
            <span>{{ fromData[scope.$index].Registration }}</span>
          </p>
        </el-form-item>
        <el-form-item label="礼品标签">
          <p style="display: inline-block" v-if="scope.row.Edit">
            <el-tooltip placement="right">
              ...
              <el-select
                v-model="fromData[scope.$index].Label"
                placeholder="请选择"
                @change="
                  GiftBasicInformationChange(
                    'Label',
                    scope.$index,
                    fromData[scope.$index].Label
                  )
                "
              >
                <el-option
          ...
          <p style="display: inline-block" v-else>
            <span v-if="fromData[scope.$index].Label == 1">家电 </span>
            <span v-if="fromData[scope.$index].Label == 2">家居 </span>
            <span v-if="fromData[scope.$index].Label == 3">生活用品</span>
            <span v-if="fromData[scope.$index].Label == 4">护肤产品</span>
            <span v-if="fromData[scope.$index].Label == 5">其他</span>
          </p>
        </el-form-item>
        <el-form-item label="剩余数量">
          <p style="display: inline-block">
            <span>{{ fromData[scope.$index].Remaining }}</span>
          </p>
        </el-form-item>
      </el-form>
    </template>
...
export default {
  ...
  methods: {
    ...
    CarouselPicturesUploadFile(params) {
      ...
      FromData.append(
        "GiftUnique",
        this.fromData[this.UploadRecordData[0]].GiftUnique
      );
      axios
        .post(Utils.ServeUrl + "/GiftManagement/ImageStoragePath", {
          GiftUnique: this.fromData[this.UploadRecordData[0]].GiftUnique,
          Registration: this.fromData[this.UploadRecordData[0]].Registration,
        })
        .then(() => {
          axios
            ...
            .then((res) => {
              this.$set(
                this.fromData[this.UploadRecordData[0]].CarouselPictures,
      ...
      axios
        ...
        .then(() => {
          axios
            ...
            .then((res) => {
              this.$set(
                this.fromData[this.UploadRecordData[0]].IntroduceImg,
                this.UploadRecordData[1],
                {
                  id: this.UploadRecordData[2],
    ...
    CarouselPicturesDelete(fromDataIndex, Id, Index) {
      ...
      this.$confirm("确认删除!", "确认信息", {
        ...
        .then(() => {
          if (Index == 0) {//暂时性删除页面的第一张 当上传后会直接替换路径
            this.$set(this.fromData[fromDataIndex].CarouselPictures, Index, {
              id: Id,
            });
            this.$message({
              type: "info",
          ...
          axios
            .post(
              ...
              {
                GiftUnique: this.fromData[fromDataIndex].GiftUnique,
                Name: Name,
              }
            )
            .then(() => {
      ...
      this.$confirm("确认删除!", "确认信息", {
        ...
        .then(() => {
          axios
            ...
            .then(() => {
              this.$set(this.fromData[fromDataIndex].Specification, Index, {
                id: Id,
                SpecificationText: "",
                InputState: false,
              });
    ...
    SpecificationAdd(fromDataIndex, Index) {
      //添加规格
      this.fromData[fromDataIndex].Specification[Index].InputState = true;
    },
    SpecificationInput(str, fromDataIndex, Id, Index) {
      ...
      this.$confirm("确认修改!", "确认信息", {
          ...
          if (str == "SpecificationTextChange") {
            axios
              .post(Utils.ServeUrl + "/GiftManagement/GiftBasicInformation", {
                ...
                Name: Name,
                SpecificationTextChange:
                  this.fromData[fromDataIndex].Specification[Index]
                    .SpecificationTextChange,
              })
              .then(() => {
                this.fromData[fromDataIndex].Specification[
                  Index
                ].SpecificationText =
                  this.fromData[fromDataIndex].Specification[
                    Index
        ...
        .catch(() => {
          ...
          this.fromData[fromDataIndex].Specification[Index].InputState = false;
          this.$message({
            type: "info",
            message: "放弃修改",
          });
      ...
      this.$confirm("确认删除!", "确认信息", {
        ...
        .then(() => {
          axios
            ...
            .then(() => {
              this.$set(this.fromData[fromDataIndex].IntroduceImg, Index, {
                id: Id,
              });
            })
            .catch((err) => {
    ...
    GiftBasicInformationChange(Name, fromDataIndex, ChangeText) {
      ...
      this.$confirm("确认修改!", "确认信息", {
            ...
            .then(() => {
              switch (Name) {
                case "CommodityFunllName":
                  this.fromData[fromDataIndex].CommodityFunllName = ChangeText;
                  this.fromData[fromDataIndex].CommodityFunllNameChange =
                    ChangeText;
                  break;
                case "CommodityName":
                  this.fromData[fromDataIndex].CommodityName = ChangeText;
                  this.fromData[fromDataIndex].CommodityNameChange = ChangeText;
                  break;
                case "BusinessName":
                  this.fromData[fromDataIndex].BusinessName = ChangeText;
                  this.fromData[fromDataIndex].BusinessNameChange = ChangeText;
                  break;
              }
            })
            .catch((err) => {
        ...
        .catch(() => {
          switch (Name) {
            case "CommodityFunllName":
              this.fromData[fromDataIndex].CommodityFunllName =
                this.fromData[fromDataIndex].CommodityFunllNameChange;
              break;
            case "CommodityName":
              this.fromData[fromDataIndex].CommodityName =
                this.fromData[fromDataIndex].CommodityNameChange;
              break;
            case "BusinessName":
              this.fromData[fromDataIndex].BusinessName =
                this.fromData[fromDataIndex].BusinessNameChange;
              break;
          }
          this.$message({
            type: "info",
        ...
        for (let i = 0; i < DataArr.length; i++) {
          DataArr[i].CarouselPictures.forEach((item) => {
            if (item.CarouselPicturesUrl !== " ") {
              item.CarouselPicturesUrl =
                Utils.ServeUrl + item.CarouselPicturesUrl; //添加域名
            } else {
          ...
          DataArr[i].IntroduceImg.forEach((item) => {
            if (item.IntroduceImgUrl !== " ") {
              item.IntroduceImgUrl = Utils.ServeUrl + item.IntroduceImgUrl; //添加域名
            } else {
